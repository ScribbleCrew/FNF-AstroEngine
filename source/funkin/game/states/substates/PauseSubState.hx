...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		levelDifficulty.setFormat(Constants.DEFAULT_FONT, 32);
		levelDifficulty.updateHitbox();
		add(levelDifficulty);

		var blueballedTxt:FlxText = new FlxText(20, 15 + 64, 0, "Blueballed: " + PlayState.deathCounter, 32);
		blueballedTxt.scrollFactor.set();
		blueballedTxt.setFormat(Constants.DEFAULT_FONT, 32);
		blueballedTxt.updateHitbox();
		add(blueballedTxt);

		practiceText = new FlxText(20, 15 + 101, 0, "Practice Mode", 32);
		practiceText.scrollFactor.set();
		practiceText.setFormat(Constants.DEFAULT_FONT, 32);
		...
		chartingText.setFormat(Constants.DEFAULT_FONT, 32);
		...
		if ((controls.ACCEPT|| FlxG.mouse.justPressedMiddle) && (cantUnpause <= 0))
		{
			...
			switch (daSelected)
			{
				...
				case 'Toggle Botplay':
					...
					PlayState.changedDifficulty = true;
					PlayState.instance.baseUI.botplayTxt.visible = PlayState.instance.cpuControlled;
					PlayState.instance.baseUI.botplayTxt.alpha = 1;
					PlayState.instance.baseUI.botplaySine = 0;
				case "Exit to menu":
	...
	function changeSelection(change:Int = 0, ?snd:Bool = true):Void
	{
	...
	function regenMenu():Void
	{
		...
		for (num => str in menuItems)
		{
			...
			if (str == 'Skip Time')
			{
				...
				skipTimeText.setFormat(Constants.DEFAULT_FONT, 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
