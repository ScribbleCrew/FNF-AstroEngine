...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var grpBlacks:FlxTypedGroup<AttachedFlxSprite>;
	var grpOptions:FlxTypedGroup<Alphabet>;
	var grpBinds:FlxTypedGroup<Alphabet>;
	var selectSpr:AttachedFlxSprite;
	...
	public function new()
	{
		...
		add(grpOptions);
		grpBlacks = new FlxTypedGroup<AttachedFlxSprite>();
		add(grpBlacks);
		selectSpr = new AttachedFlxSprite();
		...
		controllerSpr = new FlxSprite(50, 40).loadGraphic(Paths.image('ui/input_type','embed'), true, 82, 60);
	...
	function createTexts()
	{
		...
		grpBlacks.forEachAlive(function(black:AttachedFlxSprite) black.destroy());
			...
			if (onKeyboardMode || option[0])
			{
				if (option.length > 1)
				{
					...
					var text:Alphabet = new Alphabet(475, 300,
						!isDisplayKey ? str : str, !isDisplayKey);
	...
	function addKeyText(text:Alphabet, option:Array<Dynamic>, id:Int)
	{
		...
		for (n in 0...2)
		{
			...
			var black:AttachedFlxSprite = new AttachedFlxSprite();
