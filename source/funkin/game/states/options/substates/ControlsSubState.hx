...
class ControlsSubState extends BaseMenu
{
	...
	public function new()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			var optionText:funkin.game.objects.Alphabet = new funkin.game.objects.Alphabet(200, 300, optionShit[i][0], (!isCentered || isDefaultKey));
	...
	override function update(elapsed:Float)
	{
		if (!rebindingKey)
		{
			...
			if ((controls.ACCEPT || FlxG.mouse.justPressedMiddle) && nextAccept <= 0)
			{
				...
				{
					funkin.backend.utils.ClientPrefs.keyBinds = funkin.backend.utils.ClientPrefs.defaultKeys.copy();
					reloadKeys();
					changeSelection();
		...
		else
		{
			...
			if (keyPressed > -1)
			{
				var keysArray:Array<FlxKey> = funkin.backend.utils.ClientPrefs.keyBinds.get(optionShit[curSelected][1]);
				...
				funkin.backend.utils.ClientPrefs.keyBinds.set(optionShit[curSelected][1], keysArray);
	...
	function reloadKeys()
	{
		...
		trace('Reloaded keys: ' + funkin.backend.utils.ClientPrefs.keyBinds);
