...
class NotesSubState extends BaseMenu
{
	...
	public function new() {
		...
		WindowUtil.setTitle("Note Colors");
		#end
		...
		copyButton = new FlxSprite(760, 50).loadGraphic(Paths.image('noteMenu/copy', 'embed'));
		copyButton.alpha = 0.6;
		add(copyButton);

		pasteButton = new FlxSprite(1180, 50).loadGraphic(Paths.image('noteMenu/paste', 'embed'));
		...
		colorPalette = new FlxSprite(820, 580).loadGraphic(Paths.image('noteMenu/palette', 'embed', false));
		colorPalette.scale.set(20, 20);
		colorPalette.updateHitbox();
		colorPalette.antialiasing = false;
		add(colorPalette);
		
		colorWheel = new FlxSprite(860, 200).loadGraphic(Paths.image('noteMenu/colorWheel', 'embed'));
		...
		tip.setFormat(Constants.DEFAULT_FONT, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip.borderSize = 2;
		add(tip);

		tipTxt = new FlxText(tipX, tipY + 24, 0, '', 16);
		tipTxt.setFormat(Constants.DEFAULT_FONT, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipTxt.borderSize = 2;
		add(tipTxt);
		updateTip();
	...
	override function update(elapsed:Float) {
		...
		if(generalPressed)
		{
			...
			else if (pointerOverlaps(myNotes))
			{
				...
				setShaderColor(colorPalette.pixels.getPixel(
	...
	public function spawnNotes()
	{
		...
		skinNote = new FlxSprite(48, 24).loadGraphic(Paths.image('noteMenu/' + (onPixel ? 'note' : 'notePixel'), 'embed'), true, res, res);
		...
		for (i in 0...3)
		{
			var newNote:FlxSprite = new FlxSprite(230 + (100 * i), 100).loadGraphic(Paths.image('noteMenu/' + (!onPixel ? 'note' : 'notePixel'), 'embed'), true, res, res);
	...
	function updateColors(specific:Null<FlxColor> = null)
	{
		...
		for (letter in [alphabetHex.letters, alphabetR.letters, alphabetG.letters,alphabetB.letters])
			for(i in 0...letter.length)
				letter[i].color = color;
