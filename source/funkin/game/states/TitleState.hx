...
class TitleState extends MusicBeatState
{
	...
	function loadXMLData():Void
	{
		...
		try
		{
			...
			if (xml != null)
			{
				...
				if (node.name != null)
					logoPos.set(Std.parseFloat(node.has.x ? node.att.x : null).getDefault(-150), Std.parseFloat(node.has.y ? node.att.y : null).getDefault(-100));
	...
	*/
	...
	@:dox(hide) function get_introTextList():Array<Array<String>>
		return [for (i in #if MODS_ALLOWED Mods.mergeAllTextsNamed('data/introText.txt') #else Assets.getText(Paths.txt('introText')).split('\n') #end) i.split('--')];
	...
	*/
	...
	@:dox(hide) override function update(elapsed:Float):Void
	{
		...
		#if ASTRO_WATERMARKS if (FlxG.keys.justPressed.SEVEN) MusicBeatState.switchState("VeryFuniState", [new TitleState()]); /* YES, FINALLY IT WORKS!!?!?!?!? */ #end
