...
class CharacterEditorState extends MusicBeatState implements FlxUIEventHandler.FlxUIEvent
{
	...
	public function new(char:String = null, goToPlayState:Bool = true)
	{
		...
		if (this._char == null)
			this._char = Constants.DEFAULT_CHARACTER;
	...
	function makeUIMenu()
	{
		UI_box = new FlxUIBox(FlxG.width - 275, 25, 250, 120, ['Ghost', 'Settings']);
		UI_box.scrollFactor.set();
		UI_box.cameras = [camHUD];

		UI_characterbox = new FlxUIBox(UI_box.x - 100, UI_box.y + UI_box.height + 10, 350, 280, ['Animations', 'Character']);
	...
	function addGhostUI()
	{
		...
		// var hideGhostButton:FlxUIButton = null;
		var makeGhostButton:FlxUIButton = new FlxUIButton(25, 15, "Make Ghost", function()
		{
		...
		/*hideGhostButton = new FlxUIButton(20 + makeGhostButton.width, makeGhostButton.y, "Hide Ghost", function() {
				ghost.visible = false;
		...
		var highlightGhost:FlxUICheckBox = new FlxUICheckBox(20 + makeGhostButton.x + makeGhostButton.width, makeGhostButton.y, "Highlight Ghost", 100);
		...
		var ghostAlphaSlider:FlxUISlider = new FlxUISlider(15, makeGhostButton.y + 25, function(v:Float)
		{
			ghostAlpha = v;
			ghost.alpha = ghostAlpha;
			if (animateGhost != null)
	...
	function addSettingsUI()
	{
		...
		check_player = new FlxUICheckBox(10, 60, "Playable Character", 100);
		check_player.checked = character.isPlayer;
		...
		var reloadCharacter:FlxUIButton = new FlxUIButton(140, 20, "Reload Char", function()
		{
			addCharacter(true);
			updatePointerPos();
		...
		var templateCharacter:FlxUIButton = new FlxUIButton(140, 50, "Load Template", function()
		{
			final _template:Character.CharacterFile = {
				animations: [
	...
	function addAnimationsUI()
	{
		...
		var addUpdateButton:FlxUIButton = new FlxUIButton(70, animationIndicesInputText.y + 60, "Add/Update", function()
		{
		...
		var removeButton:FlxUIButton = new FlxUIButton(180, animationIndicesInputText.y + 60, "Remove", function()
		{
			for (anim in character.animationsArray)
				if (animationInputText.text == anim.anim)
				{
	...
	function addCharacterUI()
	{
		...
		var reloadImage:FlxUIButton = new FlxUIButton(imageInputText.x + 210, imageInputText.y - 3, "Reload Image", function()
		{
		...
		var decideIconColor:FlxUIButton = new FlxUIButton(reloadImage.x, reloadImage.y + 30, "Get Icon Color", function()
		{
		...
		flipXCheckBox = new FlxUICheckBox(singDurationStepper.x + 80, singDurationStepper.y, "Flip X", 50);
		...
		noAntialiasingCheckBox = new FlxUICheckBox(flipXCheckBox.x, flipXCheckBox.y + 40, "No Antialiasing", 80);
		...
		positionYStepper = new FlxUINumericStepper(positionXStepper.x + 70, positionXStepper.y, 10, character.positionArray[1], -9000, 9000, 0);
		...
		var saveCharacterButton:FlxUIButton = new FlxUIButton(reloadImage.x, noAntialiasingCheckBox.y + 40, "Save Character", function()
		{
			saveCharacter();
		});
	...
	public function UIEvent(id:String, sender:Dynamic)
	{
		...
		if (id == FlxUIInputText.CHANGE_EVENT)
		{
			if (sender == healthIconInputText)
			{
				...
				#if DISCORD_ALLOWED if (lastIcon != healthIcon.character)
					updatePresence(); #end
				unsavedProgress = true;
			}
		...
		else if (FlxG.keys.justPressed.ESCAPE)
		{
			...
			if (!_goToPlayState)
			{
				if (!unsavedProgress)
				{
					...
					openSubState(new Prompt.ExitConfirmationPrompt());
			}
			else
	...
	inline function updateHealthBar()
	{
		...
		#if DISCORD_ALLOWED updatePresence(); #end
	}

	#if DISCORD_ALLOWED
	inline function updatePresence():Void
	...
	function saveCharacter()
	{
		...
		var data:String = AstroJsonPrinter.print(json, ['offsets', 'position', 'healthbar_colors', 'camera_position', 'indices']);
