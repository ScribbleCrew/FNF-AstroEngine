...
import funkin.backend.lunarUI.WindowPopup;
...
class ChartingState extends UIState implements FlxUIEventHandler.FlxUIEvent {
	...
	inline static final __BACKUP_EXT = '.fnfBackup';
	...
	var events:Array<MetaNote.EventMetaNote> = [];
	...
	override function create() {
		...
		chartEditorSave.bind('chart_editor_data', CoolUtil.savePath);
		...
		infoBox = new FlxUIBox(infoBoxPosition.x, infoBoxPosition.y, 220, 220, ['Information']);
		...
		mainBox = new FlxUIBox(mainBoxPosition.x, mainBoxPosition.y, 300, 280, ['Charting', 'Data', 'Events', 'Note', 'Section', 'Song']);
		...
		upperBox = new FlxUIBox(40, 40, 330, 300, ['File', 'Edit', 'View']);
	...
	override function update(elapsed:Float) {
		if (!fileDialog.completed) {
			lastFocus = FlxUIInputText.focusOn;
			return;
		}
		...
		if (autoSaveCap > 0) {
			...
			if (autoSaveTime >= autoSaveCap #if debug || FlxG.keys.justPressed.NUMPADMULTIPLY #end) {
				...
				if (!FileUtil.isDir('backups'))
					FileUtil.createDirectory('backups');
				File.saveContent('backups/$chartName.$__BACKUP_EXT', dataToSave);

				if (backupLimit > 0) {
					var files:Array<String> = FileSystem.readDirectory('backups/').filter((file:String) -> file.endsWith('.$__BACKUP_EXT'));
		...
		if (FlxG.sound.music != null) {
			if (FlxUIInputText.focusOn == null) // If not typing anything
			{
		...
		if (FlxUIInputText.focusOn == null && lastFocus == null) {
			...
			if (FlxG.keys.justPressed.ENTER) {
				...
				else if (FlxG.keys.justPressed.V) // Paste (Ctrl + V)
				{
					if (copiedNotes.length > 0 || copiedEvents.length > 0) {
						...
						var pushedEvents:Array<MetaNote.EventMetaNote> = [];
						for (note in selectedNotes) {
							if (note == null)
								continue;
							...
							if (!note.isEvent) {
								...
								pushedEvents.push(cast(note, MetaNote.EventMetaNote));
						}
				...
				|| (isMovingNotes && (FlxG.mouse.justPressedRight || FlxG.keys.justPressed.ESCAPE))) // Delete button
			{
				if (selectedNotes.length > 0) {
					...
					var removedEvents:Array<MetaNote.EventMetaNote> = [];
					while (selectedNotes.length > 0) {
						...
						if (!note.isEvent) {
							...
							var ev:MetaNote.EventMetaNote = cast(note, MetaNote.EventMetaNote);
							events.remove(ev);
		...
		if (Constants.SHOW_EVENT_COLUMN && lockedEvents)
			minX += Constants.GRID_SIZE;
			...
			if (isMovingNotes) {
				...
				if (dummyArrow.y != movingNotesLastY) {
					...
					for (note in movingNotes) // Try to figure out new strum time for the notes, DEFINITELY INACCURATE WITH BPM CHANGING, ALTHOUGH UNTESTED
					{
						...
						if (note.isEvent)
							cast(note, MetaNote.EventMetaNote).updateEventText();
					}
				...
				if (FlxG.keys.pressed.CONTROL && FlxG.mouse.justPressed) {
					...
					if (closest != null && (!closest.isEvent || !lockedEvents)) {
						...
						else if (!FlxG.keys.pressed.CONTROL) // Remove Note/Event
						{
							...
							if (!closest.isEvent)
								...
								events.remove(cast(closest, MetaNote.EventMetaNote));
					...
					else if (!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height) // Add note
					{
						...
						if (noteData >= 0) {
							...
							var eventAdded:MetaNote.EventMetaNote = createEvent([
								strumTime,
								[
									[
	...
	function moveSelectedNotes(noteData:Int = 0, lastY:Float) // This turns selected notes into moving notes
	{
		...
		var originalEvents:Array<MetaNote.EventMetaNote> = [];
		var movedNotes:Array<MetaNote> = [];
		var movedEvents:Array<MetaNote.EventMetaNote> = [];
		for (note in selectedNotes) {
			if (note == null)
				continue;
			...
			if (!note.isEvent) {
				...
				events.remove(cast(note, MetaNote.EventMetaNote));
				originalEvents.push(cast(note, MetaNote.EventMetaNote));
				var mov:MetaNote.EventMetaNote = createEvent(note.songData);
				movingNotes.add(mov);
	...
	function updateScrollY() {
		...
		scrollY = (((Conductor.songPosition - secStartTime) / secCrochet * Constants.GRID_SIZE * 4)
			+ (secRows * Constants.GRID_SIZE)) * curZoom
			- FlxG.height / 2;
	}
	...
	function onSelectNote() {
		if (selectedNotes.length == 1) // Only one note selected
		{
			...
			{
				var eventNote:MetaNote.EventMetaNote = cast(selectedNotes[0], MetaNote.EventMetaNote);
	...
	function loadMusic(?killAudio:Bool = false) {
		...
		#end
		Windows.title = ('%{GAME_TITLE} - Chart Editor - ${PlayState.SONG.song} (${Difficulty.list[PlayState.storyDifficulty]})');
		#end
	...
	function createEvent(event:Dynamic) {
		...
		var swagEvent:MetaNote.EventMetaNote = new MetaNote.EventMetaNote(daStrumTime, event);
	...
	var playbackSlider:FlxUISlider;

	var mouseSnapCheckBox:FlxUICheckBox;
	var ignoreProgressCheckBox:FlxUICheckBox;
	var hitsoundPlayerStepper:FlxUINumericStepper;
	var hitsoundOpponentStepper:FlxUINumericStepper;
	var metronomeStepper:FlxUINumericStepper;

	var instVolumeStepper:FlxUINumericStepper;
	var instMuteCheckBox:FlxUICheckBox;
	var playerVolumeStepper:FlxUINumericStepper;
	var playerMuteCheckBox:FlxUICheckBox;
	var opponentVolumeStepper:FlxUINumericStepper;
	var opponentMuteCheckBox:FlxUICheckBox;
	...
	function addChartingTab() {
		...
		objY += 25;
		playbackSlider = new FlxUISlider(50, objY, function(v:Float) setPitch(playbackRate = v), 1, 0.5, 3, 200);
		playbackSlider.label = 'Playback Rate';

		objY += 60;
		mouseSnapCheckBox = new FlxUICheckBox(objX, objY, 'Mouse Scroll Snap', 100,
		...
		ignoreProgressCheckBox = new FlxUICheckBox(objX + 150, objY, 'Ignore Progress Warnings', 100,
		...
		objY += 50;
		hitsoundPlayerStepper = new FlxUINumericStepper(objX, objY, 0.2, 0, 0, 1, 1);
		hitsoundOpponentStepper = new FlxUINumericStepper(objX + 100, objY, 0.2, 0, 0, 1, 1);
		metronomeStepper = new FlxUINumericStepper(objX + 200, objY, 0.2, 0, 0, 1, 1);

		objY += 50;
		instVolumeStepper = new FlxUINumericStepper(objX, objY, 0.1, 0.6, 0, 1, 1);
		instVolumeStepper.onValueChange = updateAudioVolume;
		playerVolumeStepper = new FlxUINumericStepper(objX + 100, objY, 0.1, 1, 0, 1, 1);
		playerVolumeStepper.onValueChange = updateAudioVolume;
		opponentVolumeStepper = new FlxUINumericStepper(objX + 200, objY, 0.1, 1, 0, 1, 1);
		opponentVolumeStepper.onValueChange = updateAudioVolume;

		objY += 25;
		instMuteCheckBox = new FlxUICheckBox(objX, objY, 'Mute', 60, updateAudioVolume);
		playerMuteCheckBox = new FlxUICheckBox(objX + 100, objY, 'Mute', 60, updateAudioVolume);
		opponentMuteCheckBox = new FlxUICheckBox(objX + 200, objY, 'Mute', 60, updateAudioVolume);
	...
	var gameOverSndInputText:FlxUIInputText;
	var gameOverLoopInputText:FlxUIInputText;
	var gameOverRetryInputText:FlxUIInputText;
	var noRGBCheckBox:FlxUICheckBox;
	var noteTextureInputText:FlxUIInputText;
	...
	function addDataTab() {
		...
		objY += 40;
		gameOverSndInputText = new FlxUIInputText(objX, objY, 120, '', 8);
		...
		objY += 40;
		gameOverLoopInputText = new FlxUIInputText(objX, objY, 120, '', 8);
		...
		objY += 40;
		gameOverRetryInputText = new FlxUIInputText(objX, objY, 120, '', 8);
		...
		objY += 35;
		noRGBCheckBox = new FlxUICheckBox(objX, objY, 'Disable Note RGB', 100, updateNotesRGB);

		objY += 40;
		noteTextureInputText = new FlxUIInputText(objX, objY, 120, '');
		...
		noteSplashesInputText = new FlxUIInputText(objX + 140, objY, 120, '');
	...
	function addEventsTab() {
		...
		eventDropDown = new FlxUIDropDownMenu(objX, objY, [], function(id:Int, character:String) {
			...
			if (selectedNotes.length > 1) {
				for (note in selectedNotes) {
					...
					var event:MetaNote.EventMetaNote = cast(note, MetaNote.EventMetaNote);
				...
				var event:MetaNote.EventMetaNote = cast(selectedNotes[0], MetaNote.EventMetaNote);
				event.events[Std.int(FlxMath.bound(curEventSelected, 0, event.events.length - 1))][0] = eventName;
				event.updateEventText();
			}
		});

		function genericEventButton(func:MetaNote.EventMetaNote->Void) {
			if (selectedNotes.length == 1) {
		...
		var objX2 = 140;
		var removeButton:FlxUIButton = new FlxUIButton(objX2, objY, '-', function() {
			genericEventButton(function(event:MetaNote.EventMetaNote) {
		...
		var addButton:FlxUIButton = new FlxUIButton(objX2 + 30, objY, '+', function() {
			genericEventButton(function(event:MetaNote.EventMetaNote) {
				event.events.push([
					eventsList[Std.int(Math.max(eventDropDown.selectedIndex, 0))][0],
					value1InputText.text,
		...
		var leftButton:FlxUIButton = new FlxUIButton(objX2 + 80, objY, '<', function() {
			genericEventButton(function(event:MetaNote.EventMetaNote) curEventSelected = FlxMath.wrap(curEventSelected - 1, 0, event.events.length - 1));
		}, 20);
		var rightButton:FlxUIButton = new FlxUIButton(objX2 + 110, objY, '>', function() {
			genericEventButton(function(event:MetaNote.EventMetaNote) curEventSelected = FlxMath.wrap(curEventSelected + 1, 0, event.events.length - 1));
		}, 20);
		...
		objY += 70;
		value1InputText = new FlxUIInputText(objX, objY, 120, '', 8);
		value1InputText.onChange = function(old:String, cur:String) changeEventsValue(cur, 1);
		value2InputText = new FlxUIInputText(objX + 150, objY, 120, '', 8);
	...
	var susLengthStepper:FlxUINumericStepper;
	...
	function addNoteTab() {
		...
		susLengthStepper = new FlxUINumericStepper(objX, objY, Conductor.stepCrochet / 2, 0, 0, Conductor.stepCrochet * 128, 1, 80);
		...
		objY += 40;
		strumTimeStepper = new FlxUINumericStepper(objX, objY, Conductor.stepCrochet, 0, -5000, Math.POSITIVE_INFINITY, 3, 120);
	...
	var mustHitCheckBox:FlxUICheckBox;
	var gfSectionCheckBox:FlxUICheckBox;
	var altAnimSectionCheckBox:FlxUICheckBox;

	var changeBpmCheckBox:FlxUICheckBox;
	var changeBpmStepper:FlxUINumericStepper;
	var beatsPerSecStepper:FlxUINumericStepper;

	function addSectionTab() {
		var affectNotes:FlxUICheckBox = null;
		var affectEvents:FlxUICheckBox = null;
		var copyLastSecStepper:FlxUINumericStepper = null;
		...
		mustHitCheckBox = new FlxUICheckBox(objX, objY, 'Must Hit Sec.', 70, function() {
			var sec = getCurChartSection();
			if (sec != null)
				sec.mustHitSection = mustHitCheckBox.checked;
			updateHeads(true);
		});
		gfSectionCheckBox = new FlxUICheckBox(objX + 100, objY, 'GF Section', 70, function() {
			var sec = getCurChartSection();
			if (sec != null)
				sec.gfSection = gfSectionCheckBox.checked;
			updateHeads(true);
		});
		altAnimSectionCheckBox = new FlxUICheckBox(objX + 200, objY, 'Alt Anim', 70, function() {
			var sec = getCurChartSection();
			if (sec != null)
		...
		objY += 40;
		changeBpmCheckBox = new FlxUICheckBox(objX, objY, 'Change BPM', 80, function() {
			var sec = getCurChartSection();
			if (sec != null) {
		...
		objY += 25;
		changeBpmStepper = new FlxUINumericStepper(objX, objY, 1, 0, 1, 400, 3);
		...
		beatsPerSecStepper = new FlxUINumericStepper(objX + 150, objY, 1, 4, 1, 7, 2);
		...
		objY += 40;
		var copyButton:FlxUIButton = new FlxUIButton(objX, objY, 'Copy Section', copyNotesOnSection.bind());
		var pasteButton:FlxUIButton = new FlxUIButton(objX + 100, objY, 'Paste Section', function() {
			pasteCopiedNotesToSection(affectNotes.checked, affectEvents.checked);
		});
		var clearButton:FlxUIButton = new FlxUIButton(objX + 200, objY, 'Clear', function() {
			if (affectNotes.checked) {
				for (note in curRenderedNotes) {
					if (note == null || note.isEvent)
						continue;
			...
			if (affectEvents.checked) {
				for (event in curRenderedNotes) {
					...
					selectedNotes.remove(event);
					events.remove(cast(event, MetaNote.EventMetaNote));
				}
			}
			softReloadNotes(true);
		});
		...
		objY += 25;
		affectNotes = new FlxUICheckBox(objX, objY, 'Notes', 60);
		affectNotes.checked = true;
		affectEvents = new FlxUICheckBox(objX + 100, objY, 'Events', 60);

		objY += 32;
		var copyLastSecButton:FlxUIButton = new FlxUIButton(objX, objY, 'Copy Last Section', function() {
		...
		copyLastSecStepper = new FlxUINumericStepper(objX + 110, objY + 2, 1, 1, -999, 999, 0);
		...
		var duetSectionButton:FlxUIButton = new FlxUIButton(objX + 100, objY, 'Duet Section', function() {
			var side:Int = -1;
			for (note in curRenderedNotes.members) {
				...
				if (side > -1) {
					...
					side = Math.floor(note.songData[1] / Constants.GRID_COLUMNS_PER_PLAYER);
			}
		...
		var mirrorNotesButton:FlxUIButton = new FlxUIButton(objX + 200, objY, 'Mirror Notes', function() {
			...
			for (note in curRenderedNotes) {
				...
				note.changeNoteData((Math.floor(data / Constants.GRID_COLUMNS_PER_PLAYER) * Constants.GRID_COLUMNS_PER_PLAYER)
					+ Constants.GRID_COLUMNS_PER_PLAYER
					- note.noteData
					- 1);
				positionNoteXByData(note);
			}
			...
			?showMessage:Bool = true) // Used on "Paste Section" and "Copy Last Section" buttons
	{
		...
		var evs:Array<MetaNote.EventMetaNote> = [];
	...
	var audioOffsetStepper:FlxUINumericStepper;
	...
	function addSongTab() {
		...
		bpmStepper = new FlxUINumericStepper(objX, objY, 1, 1, 1, 400, 3);
		...
		scrollSpeedStepper = new FlxUINumericStepper(objX + 90, objY, 0.1, 1, 0.1, 10, 2);
		scrollSpeedStepper.onValueChange = function() PlayState.SONG.speed = scrollSpeedStepper.value;

		audioOffsetStepper = new FlxUINumericStepper(objX + 180, objY, 1, 0, -500, 500, 0);
	...
	function addFileTab() {
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  New', function() {
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Open Chart...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			fileDialog.open(function() {
				try {
					...
					{
						showOutput('Error: File loaded is not a Flx Engine/FNF 0.2.x.x chart.', true);
						return;
					}
		...
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Open Autosave...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			var fileList:Array<String> = FileSystem.readDirectory('backups/').filter((file:String) -> file.endsWith('.$__BACKUP_EXT'));
			...
			var radioGrp:FlxUIRadioGroup = new FlxUIRadioGroup(0, 0, fileList, 25, maxItems, false, 240);
			radioGrp.checked = 0;

			var hei:Float = radioGrp.height + 160;
			openSubState(new WindowPopup(420, hei, 'Choose an Autosave', function(state:WindowPopup) {
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;

				radioGrp.screenCenter(X);
				radioGrp.y = state._windowBackground.y + 80;
				radioGrp.cameras = state.cameras;
				state.add(radioGrp);

				var btn:FlxUIButton = new FlxUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', function() {
					...
					if (FileSystem.exists(path)) {
						try {
							...
							if (loadedChart == null || !Reflect.hasField(loadedChart, '__original_path')) {
								showOutput('Error: File loaded is not a valid Flx Engine autosave.', true);
								return;
							}
				...
				fileDialog.open(function() {
					try {
						...
						if (eventsFile == null || Reflect.hasField(eventsFile, 'scrollSpeed') || eventsFile.events == null) {
							showOutput('Error: File loaded is not a Flx Engine chart/events file.', true);
							return;
						}
						...
						openSubState(new WindowPopup('Events Found! Choose an action.', function(state:WindowPopup) {
							var btnY = 390;
							var btn:FlxUIButton = new FlxUIButton(0, btnY, 'Replace All', function() {
								for (event in events) {
									if (event != null) {
										event.destroy();
										selectedNotes.remove(event);
							...
							var btn:FlxUIButton = new FlxUIButton(0, btnY, 'Add', function() {
								for (event in loadedEvents)
									events.push(createEvent(event));
							...
							var btn:FlxUIButton = new FlxUIButton(0, btnY, 'Cancel', state.close);
							btn.screenCenter(X);
							btn.x += 125;
							btn.cameras = state.cameras;
							state.add(btn);
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Save', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Save as...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Reload Chart', function() {
			var func:Void->Void = function() {
				if (Song.chartPath == null) {
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Save (V-Slice)...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			fileDialog.openDirectory('Save V-Slice Chart/Metadata JSONs', function() {
				try {
					...
					openSubState(new WindowPopup('Metadata', function(state:WindowPopup) {
						var btnX = 640;
						var btnY = 400;
						var btn:FlxUIButton = new FlxUIButton(btnX, btnY, 'Save', function() {
							overwriteSavedSomething = false;
							overwriteCheck(chartFile, '$chartName-chart.json', AstroJsonPrinter.print(pack.chart, ['events', 'notes', 'scrollSpeed']),
								function() {
									overwriteCheck(metadataFile, '$chartName-metadata.json',
										AstroJsonPrinter.print(pack.metadata, ['characters', 'difficulties', 'timeChanges']), function() {
						...
						var btn:FlxUIButton = new FlxUIButton(btnX + 100, btnY, 'Cancel', state.close);
						btn.cameras = state.cameras;
						state.add(btn);

						var textX = FlxG.width / 2 - 155;
						var textY = 360;
						var artistInput:FlxUIInputText = new FlxUIInputText(textX, textY, 120, pack.metadata.artist, 8);
						artistInput.cameras = state.cameras;
						artistInput.onChange = function(old:String, cur:String) pack.metadata.artist = cur;

						var charterInput:FlxUIInputText = new FlxUIInputText(textX + 190, textY, 120, pack.metadata.charter, 8);
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Astro to V-Slice...', function() {
			...
			fileDialog.open('song.json', 'Open a Astro Engine Chart JSON', function() {
				var filePath:String = fileDialog.path.replace('/', '\\');
				var loadedChart:SwagSong = Song.parseJSON(fileDialog.data, filePath.substr(filePath.lastIndexOf('\\')));
				if (loadedChart == null || !Reflect.hasField(loadedChart, 'song')) // Check if chart is ACTUALLY a chart and valid
				{
				...
				openSubState(new WindowPopup('Metadata', function(state:WindowPopup) {
					...
					var artistInput, charterInput, difficultiesInput:FlxUIInputText = null;
							...
							if (diffs != null && diffs.length > 0) {
								...
								fileDialog.openDirectory('Save V-Slice Chart/Metadata JSONs', function() {
									...
									overwriteCheck('$path/$songName-chart.json', '$songName-chart.json',
										AstroJsonPrinter.print(pack.chart, ['events', 'notes', 'scrollSpeed']), function() {
					...
					var textY = 360;
					artistInput = new FlxUIInputText(textX, textY, 120, pack.metadata.artist, 8);
					artistInput.cameras = state.cameras;
					artistInput.onChange = function(old:String, cur:String) pack.metadata.artist = cur;

					charterInput = new FlxUIInputText(textX + 150, textY, 120, pack.metadata.charter, 8);
					charterInput.cameras = state.cameras;
					charterInput.onChange = function(old:String, cur:String) pack.metadata.charter = cur;
					...
					difficultiesInput = new FlxUIInputText(textX, textY + 42, 160, diffs.join(', '), 8);
		...
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  V-Slice to Flx...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			...
			fileDialog.open('chart.json', 'Open a V-Slice Chart file', function() {
				...
				fileDialog.open('metadata.json', 'Open a V-Slice Metadata file', function() {
					...
					try {
						var pack:AstroPackage = VSlice.convertToAstro(chart, metadata);
						if (pack.difficulties != null) {
							fileDialog.openDirectory('Save Converted Flx JSONs', function() {
								var path:String = fileDialog.path.replace('/', '\\');
								if (!path.endsWith('\\'))
									path += '\\';
								...
								function nextChart() {
									while (diffs.length > 0) {
										...
										overwriteCheck(path + chartName, chartName, AstroJsonPrinter.print(chartData, ['sectionNotes', 'events']), nextChart,
											true);
										return;
									}

									if (pack.events != null) {
										overwriteCheck(path + 'events.json', 'events.json', AstroJsonPrinter.print(pack.events, ['events']), function() {
											if (overwriteSavedSomething)
		...
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Update (Legacy)...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Preview (F12)', openEditorPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Playtest (Enter)', goToPlayState, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Exit', function() {
	...
	function addEditTab() {
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Undo', undo, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Redo', redo, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);

		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Select All', function() {
			var sel = selectedNotes;
			selectedNotes = curRenderedNotes.members.copy();
			addUndoAction(SELECT_NOTE, {old: sel, current: selectedNotes.copy()});
			onSelectNote();
		...
		if (Constants.SHOW_EVENT_COLUMN) {
			btnY++;
			btnY += 20;
			var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Lock Events', btnWid);
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Autosave Settings...', btnWid);
		btn.onClick = function() {
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;

			openSubState(new WindowPopup(400, 160, 'Autosave Settings', function(state:WindowPopup) {
				var checkbox:FlxUICheckBox = null;
				var timeStepper:FlxUINumericStepper = null;

				timeStepper = new FlxUINumericStepper(state._windowBackground.x + 50, state._windowBackground.y + 90, 1, autoSaveCap, 1, 30, 0);
				...
				checkbox = new FlxUICheckBox(timeStepper.x + 80, timeStepper.y, 'Enabled', 60, function() {
					autoSaveTime = 0;
				...
				var maxFileStepper:FlxUINumericStepper = new FlxUINumericStepper(checkbox.x + 140, checkbox.y, 1, backupLimit, 0, 50, 0);
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Clear All Notes', function() {
			var func:Void->Void = function() {
				resetSelectedNotes();
	...
	var showLastGridButton:FlxUIButton;
	var showNextGridButton:FlxUIButton;
	var noteTypeLabelsButton:FlxUIButton;
	var vortexEditorButton:FlxUIButton;
	...
	function addViewTab() {
		...
		showLastGridButton = new FlxUIButton(btnX, btnY, '', function() {
		...
		btnY += 20;
		showNextGridButton = new FlxUIButton(btnX, btnY, '', function() {
		...
		btnY++;
		btnY += 20;
		noteTypeLabelsButton = new FlxUIButton(btnX, btnY, '', function() {
		...
		btnY++;
		btnY += 20;
		vortexEditorButton = new FlxUIButton(btnX, btnY, vortexEnabled ? '  Vortex Editor ON' : '  Vortex Editor OFF', function() {
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Waveform...', function() {
			ClientPrefs.toggleVolumeKeys(false);
			openSubState(new WindowPopup(320, 200, 'Waveform Settings', function(state:WindowPopup) {
				upperBox.isMinimized = true;
				upperBox.bg.visible = false;

				var check:FlxUICheckBox = new FlxUICheckBox(state._windowBackground.x + 40, state._windowBackground.y + 80, 'Enabled', 60);
				...
				var input:FlxUIInputText = new FlxUIInputText(check.x, check.y + 50, 60, waveformC, 10);
				...
				var radioGrp:FlxUIRadioGroup = new FlxUIRadioGroup(check.x + 120, check.y, ['Instrumental', 'Main Vocals', 'Opponent Vocals']);
				radioGrp.cameras = state.cameras;
		...
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Go to...', function() {
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
			openSubState(new WindowPopup(420, 200, 'Go to Time/Section:', function(state:WindowPopup) {
				var curTime:Float = Conductor.songPosition;
				var currentSec:Int = curSec;

				var timeStepper:FlxUINumericStepper = new FlxUINumericStepper(state._windowBackground.x
					+ 100, state._windowBackground.y
					+ 90, 1,
					Math.floor(curTime) / 1000, 0, FlxG.sound.music.length / 1000
					- 0.01, 2, 80);
				timeStepper.cameras = state.cameras;
				var sectionStepper:FlxUINumericStepper = new FlxUINumericStepper(timeStepper.x + 160, timeStepper.y, 1, currentSec, 0,
				...
				var timeTxt:FlxText = new FlxText(15, state._windowBackground.y + state._windowBackground.height - 75, 230, '', 16);
				...
				var btn:FlxUIButton = new FlxUIButton(0, timeTxt.y + 30, 'Go To', function() {
					curSec = currentSec;
				...
				var btn:FlxUIButton = new FlxUIButton(0, btn.y, 'Cancel', state.close);
				btn.cameras = state.cameras;
				btn.screenCenter(X);
				btn.x += 60;
				state.add(btn);
		...
		btnY++;
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Theme...', function() {
			if (!fileDialog.completed)
				return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;

			openSubState(new funkin.backend.lunarUI.WindowPopup('Chart Editor Theme', function(state:funkin.backend.lunarUI.WindowPopup) {
				var btnY = 390;
				var btn:FlxUIButton = new FlxUIButton(0, btnY, 'Light', changeTheme.bind(LIGHT));
				btn.screenCenter(X);
				btn.x -= 125;
				btn.cameras = state.cameras;
				state.add(btn);

				var btn:FlxUIButton = new FlxUIButton(0, btnY, 'Default', changeTheme.bind(DEFAULT));
				btn.screenCenter(X);
				btn.cameras = state.cameras;
				state.add(btn);

				var btn:FlxUIButton = new FlxUIButton(0, btnY, 'Dark', changeTheme.bind(DARK));
				btn.screenCenter(X);
				btn.x += 125;
				btn.cameras = state.cameras;
				state.add(btn);
		...
		btnY += 20;
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Reset UI Boxes', function() {
			mainBox.setPosition(mainBoxPosition.x, mainBoxPosition.y);
			infoBox.setPosition(infoBoxPosition.x, infoBoxPosition.y);
			UIEvent(FlxUIBox.DROP_EVENT, btn); // to force a save
		}, btnWid);
		btn.text.alignment = LEFT;
		tab_group.add(btn);
	}
	...
	function saveChart(canQuickSave:Bool = true) {
		updateChartData();
		var chartData:String = AstroJsonPrinter.print(PlayState.SONG, ['sectionNotes', 'events']);
	...
	public function UIEvent(id:String, sender:Dynamic) {
		// trace(id, sender);
		switch (id) {
			case FlxUIButton.CLICK_EVENT, FlxUIDropDownMenu.CLICK_EVENT:
				ignoreClickForThisFrame = true;

			case FlxUIBox.CLICK_EVENT:
				ignoreClickForThisFrame = true;
				if (sender == upperBox)
					updateUpperBoxBg();

			case FlxUIBox.MINIMIZE_EVENT:
				if (sender == upperBox) {
					upperBox.bg.visible = !upperBox.isMinimized;
					updateUpperBoxBg();
				}

			case FlxUIBox.DROP_EVENT:
	...
	function actionPushNotes(dataNotes:Array<MetaNote>, dataEvents:Array<MetaNote.EventMetaNote>) {
		resetSelectedNotes();
	...
	function actionRemoveNotes(dataNotes:Array<MetaNote>, dataEvents:Array<MetaNote.EventMetaNote>) {
		if (dataNotes != null && dataNotes.length > 0)
			for (note in dataNotes)
				if (note != null) {
					notes.remove(note);
