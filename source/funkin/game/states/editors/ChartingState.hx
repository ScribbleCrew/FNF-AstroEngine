...
class ChartingState extends MusicBeatState implements FlxUIEventHandler.FlxUIEvent
{
				...
				else if(FlxG.mouse.x >= gridBg.x && FlxG.mouse.x < gridBg.x + gridBg.width)
				{
					...
					else if(!holdingAlt && FlxG.mouse.y >= gridBg.y && FlxG.mouse.y < gridBg.y + gridBg.height) // Add note
					{
						...
						else if(!lockedEvents)
						{
							...
							var eventAdded:MetaNote.EventMetaNote = createEvent([strumTime, [[eventsList[Std.int(Math.max(eventDropDown.selectedIndex, 0))][0], value1InputText.text, value2InputText.text]]]);
	...
	function addChartingTab()
	{
		...
		objY += 60;
		mouseSnapCheckBox = new FlxUICheckBox(objX, objY, 'Mouse Scroll Snap', 100, function() chartEditorSave.data.mouseScrollSnap = mouseSnapCheckBox.checked);
		mouseSnapCheckBox.checked = chartEditorSave.data.mouseScrollSnap;

		ignoreProgressCheckBox = new FlxUICheckBox(objX + 150, objY, 'Ignore Progress Warnings', 100, function() chartEditorSave.data.ignoreProgressWarns = ignoreProgressCheckBox.checked);
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Open Autosave...', function()
		{
			...
			openSubState(new BasePrompt(420, hei, 'Choose an Autosave',
				function(state:BasePrompt) {
					...
					var btn:FlxUIButton = new FlxUIButton(state.bg.x + state.bg.width - 40, state.bg.y, 'X', state.close, 40);
					...
					openSubState(new BasePrompt('Metadata',
						function(state:BasePrompt)
						{
							...
							var btn:FlxUIButton = new FlxUIButton(btnX, btnY, 'Save', function()
							{
								...
								overwriteCheck(chartFile, '$chartName-chart.json', AstroJsonPrinter.print(pack.chart, ['events', 'notes', 'scrollSpeed']), function()
								{
									overwriteCheck(metadataFile, '$chartName-metadata.json', AstroJsonPrinter.print(pack.metadata, ['characters', 'difficulties', 'timeChanges']), function()
									{
							...
							try
							{
								...
								if(diffs != null && diffs.length > 0)
								{
									...
									fileDialog.openDirectory('Save V-Slice Chart/Metadata JSONs', function()
									{
										...
										overwriteCheck('$path/$songName-chart.json', '$songName-chart.json', AstroJsonPrinter.print(pack.chart, ['events', 'notes', 'scrollSpeed']), function()
										{
											overwriteCheck('$path/$songName-metadata.json', '$songName-metadata.json', AstroJsonPrinter.print(pack.metadata, ['characters', 'difficulties', 'timeChanges']), function()
											{
												if(overwriteSavedSomething)
													showOutput('Files saved successfully to: $path!');
											});
							...
							fileDialog.openDirectory('Save Converted Flx JSONs', function()
							{
								...
								function nextChart()
								{
									while(diffs.length > 0)
									{
										...
										overwriteCheck(path + chartName, chartName, AstroJsonPrinter.print(chartData, ['sectionNotes', 'events']), nextChart, true);
										return;
									}
		...
		btn.onClick = function()
		{
			...
			openSubState(new BasePrompt(400, 160, 'Autosave Settings',
				function(state:BasePrompt)
				{
					...
					timeStepper = new FlxUINumericStepper(state.bg.x + 50, state.bg.y + 90, 1, autoSaveCap, 1, 30, 0);
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Waveform...', function()
		{
			...
			openSubState(new BasePrompt(320, 200, 'Waveform Settings',
				function(state:BasePrompt) {
					...
					var check:FlxUICheckBox = new FlxUICheckBox(state.bg.x + 40, state.bg.y + 80, 'Enabled', 60);
		...
		var btn:FlxUIButton = new FlxUIButton(btnX, btnY, '  Go to...', function()
		{
			...
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
				function(state:BasePrompt)
				{
					...
					var timeStepper:FlxUINumericStepper = new FlxUINumericStepper(state.bg.x + 100, state.bg.y + 90, 1, Math.floor(curTime)/1000, 0, FlxG.sound.music.length/1000 - 0.01, 2, 80);
					timeStepper.cameras = state.cameras;
					var sectionStepper:FlxUINumericStepper = new FlxUINumericStepper(timeStepper.x + 160, timeStepper.y, 1, currentSec, 0, PlayState.SONG.notes.length - 1, 0);
