...
class DialogueCharacterEditorState extends MusicBeatState implements FlxUIEventHandler.FlxUIEvent
{
	...
	override function create()
	{
		...
		tipText.setFormat(Constants.DEFAULT_FONT, 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipText.cameras = [camHUD];
		tipText.scrollFactor.set();
		add(tipText);

		offsetLoopText = new FlxText(10, 10, 0, '', 32);
		offsetLoopText.setFormat(Constants.DEFAULT_FONT, 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		offsetIdleText.setFormat(Constants.DEFAULT_FONT, 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function onLoadComplete(_):Void
	{
		...
		if (fullPath != null)
		{
			...
			if (rawJson != null)
			{
				var loadedChar:DialogueCharacterFile = cast tjson.TJSON.parse(rawJson);
