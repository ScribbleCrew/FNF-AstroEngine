...
import funkin.game.objects.notes.NoteSplash.NoteSplashConfig;
...
@:access(funkin.game.objects.notes.NoteSplash)
class NoteSplashEditorState extends MusicBeatState
{
    ...
    var UI:FlxUIBox;
    var properUI:FlxUIBox;
    var shaderUI:FlxUIBox;
    ...
    override function create()
    {
        ...
        UI = new FlxUIBox(0, 0, 0, 0, ["Animation"]);
        UI.canMove = UI.canMinimize = false;
        UI.y += 20;
        UI.x = FlxG.width - 300;
        UI.resize(290, 240);

        properUI = new FlxUIBox(0, 0, 0, 0, ["Properties"]);
        ...
        shaderUI = new FlxUIBox(0, 0, 0, 0, ["Shader"]);
    ...
    var templateButton:FlxUIButton;
    function addAnimTab()
    {
        ...
        numericStepperData = new FlxUINumericStepper(150, 37.5, 1, .0, .0, 999, 0);
        UI.add(numericStepperData);

        UI.add(new FlxText(150, name_input.y + 30, 0, "Indices (OPTIONAL):"));
        var indices_input:FlxUIInputText = new FlxUIInputText(150, name_input.y + 47.5, 100, "", 8);
        UI.add(indices_input);

        UI.add(new FlxText(20, 110, 0, "Minimum FPS:"));
        var minFps:FlxUINumericStepper = new FlxUINumericStepper(20, 127.5, 1, 22, 1, 120);
        UI.add(minFps);

        UI.add(new FlxText(150, 110, 0, "Maximum FPS:"));
        var maxFps:FlxUINumericStepper = new FlxUINumericStepper(150, 127.5, 1, 26, 1, 120);
        UI.add(maxFps);
        ...
        addButton = new FlxUIButton(20, 185, "Add/Update", function()
        {       
        ...
        var removeButton:FlxUIButton = new FlxUIButton(185, 185, "Remove", function()
        {
            if (config != null)
            {
    ...
    function addPropertiesTab()
    {
        ...
        imageInputText = new FlxUIInputText(60, 10, 120, imageSkin, 8);
        ...
        scaleNumericStepper = new FlxUINumericStepper(20, 57.5, 0.1, 1, 0, 4, 2, 60);
        ...
        var saveButton:FlxUIButton = new FlxUIButton(20, 130, "Save", saveSplash);
        ui.add(saveButton);

        templateButton = new FlxUIButton(20, 155, "Template");
        ui.add(templateButton);

        var loadButton:FlxUIButton = new FlxUIButton(180, 155, "Convert TXT", loadTxt);
        ui.add(loadButton);

        var allowRGBCheck:FlxUICheckBox = new FlxUICheckBox(20, 105, "", 1);
        function check()
        {
            if (config != null)
        ...
        var allowPixelCheck:FlxUICheckBox = new FlxUICheckBox(allowRGBCheck.x + 110, allowRGBCheck.y, "", 1);
        function check()
        {
            if (config != null)
    ...
    var defaultButton:FlxUICheckBox;
    function addShadersTab()
    {
        ...
        var red = new FlxUINumericStepper(60, 30, 1, redShader[0], 0, 255, 0);
        red.onValueChange = () -> {
        ...
        var green = new FlxUINumericStepper(60, 50, 1, redShader[1], 0, 255, 0);
        green.onValueChange = () -> {
            var shader = switch (changeShader.selectedLabel)
            {
        ...
        var blue = new FlxUINumericStepper(60, 70, 1, redShader[2], 0, 255, 0);
        blue.onValueChange = () -> {
            var shader = switch (changeShader.selectedLabel)
            {
        ...
        defaultButton = new FlxUICheckBox(shaderUI.x + 30, shaderUI.y + 115, "Do not replace", 100, () -> onCheck());
    ...
    { 
        ...
        var blockInput:Bool = FlxUIInputText.focusOn != null;
    ...
    function onLoadComplete(_):Void
    {
        ...
        try 
        {
            var txtLoaded:Dynamic = tjson.TJSON.parse(Json.stringify(_file));
            var txt:String = null;
