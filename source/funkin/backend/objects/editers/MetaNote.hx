...
class MetaNote extends Note
{
	...
	{
		this.songData[1] = v;
		this.noteData = v % Constants.GRID_COLUMNS_PER_PLAYER;
		...
		updateHitbox();
		if(width > height)
			setGraphicSize(Constants.GRID_SIZE);
		else
			setGraphicSize(0, Constants.GRID_SIZE);
	...
	public function setSustainLength(v:Float, stepCrochet:Float, zoom:Float = 1)
	{
		...
		if(sustainLength > 0)
		{
			...
			sustainSprite.setGraphicSize(8, Math.max(0, (v * Constants.GRID_SIZE / stepCrochet * zoom) + Constants.GRID_SIZE/2));
	...
	public function findNoteTypeText(num:Int)
	{
		...
		if(num != 0)
		{
			...
			{
				txt = new FlxText(0, 0, Constants.GRID_SIZE, (num > 0) ? Std.string(num) : '?', 16);
				txt.autoSize = false;
...
class EventMetaNote extends MetaNote
{
	...
	public function new(time:Float, eventData:Dynamic)
	{
		...
		eventText.setFormat(Constants.DEFAULT_FONT, 12, FlxColor.WHITE, RIGHT);
