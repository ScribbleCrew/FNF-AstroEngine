...
class FlxUIButton extends FlxSpriteGroup
{
	...
	public var clickStyle:FlxUIBox.UIStyleData = {
		bgColor: FlxColor.BLACK,
		textColor: FlxColor.WHITE,
		bgAlpha: 1
	};
	public var hoverStyle:FlxUIBox.UIStyleData = {
		bgColor: FlxColor.WHITE,
		textColor: FlxColor.BLACK,
		bgAlpha: 1
	};
	public var normalStyle:FlxUIBox.UIStyleData = {
		bgColor: 0xFFAAAAAA,
		textColor: FlxColor.BLACK,
		bgAlpha: 1
	};
	...
	override function update(elapsed:Float)
	{
		...
		if(forceCheckNext || FlxG.mouse.justMoved || FlxG.mouse.justPressed)
		{
			...
			if(!isClicked)
			{
				var style:FlxUIBox.UIStyleData = (overlapped) ? hoverStyle : normalStyle;
			...
			if(overlapped && FlxG.mouse.justPressed)
			{
				...
				if(broadcastButtonEvent) FlxUIEventHandler.event(CLICK_EVENT, this);
			}
		}
	}
