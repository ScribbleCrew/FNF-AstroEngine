...
class FlxUICheckBox extends FlxSpriteGroup
{
	...
	{
		box.loadGraphic(Paths.image('ui/checkbox', 'embed'), true, 16, 16);
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.mouse.justPressed)
		{
				...
				(mousePos.y >= screenPos.y && mousePos.y < screenPos.y + height))
			{
				...
				if(broadcastCheckBoxEvent) FlxUIEventHandler.event(CLICK_EVENT, this);
			}
		}
	}
