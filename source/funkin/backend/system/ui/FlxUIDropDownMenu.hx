...
class FlxUIDropDownMenu extends FlxUIInputText
{
	...
	public function new(x:Float, y:Float, list:Array<String>, callback:Int->String->Void, ?width:Float = 100)
	{
		...
		button = new FlxSprite(behindText.width + 1, 0).loadGraphic(Paths.image('ui/dropdown_button', 'embed'), true, 20, 20);
	...
	{
		var lastFocus = FlxUIInputText.focusOn;
		...
		if(FlxG.mouse.justPressed)
		{
			if(FlxG.mouse.overlaps(button, camera))
			{
				...
				if(lastFocus != this)
					FlxUIInputText.focusOn = this;
				else if(FlxUIInputText.focusOn == this)
					FlxUIInputText.focusOn = null;
			}
		}
		...
		if(lastFocus != FlxUIInputText.focusOn)
		{
			showDropDown(FlxUIInputText.focusOn == this);
		}
		else if(FlxUIInputText.focusOn == this)
		{
	...
	function clickedOn(num:Int, label:String)
	{
		...
		if(broadcastDropDownEvent) FlxUIEventHandler.event(CLICK_EVENT, this);
	}
