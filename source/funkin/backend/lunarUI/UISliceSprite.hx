...
class UISliceSprite extends UIObject {
	...
	@:noCompletion public function centerScreen(?axes:FlxAxes = XY):UISliceSprite {
		if (axes.x)
	...
	public function centerSpriteOnThis(spr:FlxObject, ?axes:FlxAxes = XY) : FlxObject
	{
		if(axes.x)
			spr.x = (bWidth - spr.width) / 2;
		if(axes.y)
			spr.y = (bHeight - spr.height) / 2;
		return spr;
	}
	...
	public override function draw():Void @:privateAccess {
